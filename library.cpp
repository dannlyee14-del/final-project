...
#include <windows.h>
#include <conio.h>
#include "library.h"     // 搬移到 Windows 系統標頭檔之後
...
char Library::getKey() {
    ...
    // 防禦性安全防護：如果讀取到不相容環境的錯誤碼，阻擋高頻刷新，避免黑屏
    if (key == -1 || key == 255) {
        Sleep(50); 
        return ' ';
    }

    // 擴充：完美支援 Windows 11 控制台與 Windows Terminal 的 224 和 0 前綴
    if (key == 224 || key == 0) {
        int k = _getch();
        if (k == 72) return 'U'; // 上方向鍵
        if (k == 80) return 'D'; // 下方向鍵
        if (k == 77) return 'R'; // 右方向鍵
        if (k == 75) return 'L'; // 左方向鍵
    }
